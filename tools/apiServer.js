...
server.post("/novels/", function (req, res, next) {
  const error = validatenovel(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.name); // Generate a slug for new novels.
    next();
  }
});
...
function validatenovel(novels) {
  if (!novels.name) return "Name is required.";
  if (!novels.authorId) return "Author is required.";
  if (!novels.description) return "Description is required.";
  return "";
}
